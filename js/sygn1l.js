...
(() => {
  ...
  async function syncOnSignIn(localSnapshot=null){
    try{
      const cloud = await loadCloud();

      if (cloud?.cloudState){
        const cloudUpdated = Number(cloud.cloudUpdated || 0);
        const localUpdated = Number(localSnapshot?.updatedAtMs || 0);

        // Choose the newer state to prevent "rollback" when local autosaves are
        // more frequent than cloud throttled saves.
        if (localSnapshot?.raw && localUpdated > (cloudUpdated + 2000)){
          mergeCloudState(localSnapshot.raw);
          pushLog("log","SYS","LOCAL NEWER THAN CLOUD. USING LOCAL STATE.");
          applyOfflineFromLastSeen("SYS");
          await saveCloud(true);
        } else {
          mergeCloudState(cloud.cloudState);
          // Apply offline based on the chosen state's lastSeenMs
          applyOfflineFromLastSeen("SYS");
  ...
  async function initAuth(localSnapshot=null){
    if (!supabase){
    ...
    if (session?.user?.id){
      ...
      cloudReady = true;
      applySignedInUI();
      await syncOnSignIn(localSnapshot);
    } else {
      userId = null;
      cloudReady = false;
      applySignedOutUI();
    ...
    supabase.auth.onAuthStateChange(async (_evt, session2) => {
      if (session2?.user?.id){
        ...
        cloudReady = true;
        applySignedInUI();
        // Build a fresh local snapshot at the moment of signing in
        const snap = {
          updatedAtMs: Number(state.updatedAtMs || 0),
          lastSeenMs: Number(state.lastSeenMs || 0),
          raw: JSON.parse(JSON.stringify(state))
        };
        await syncOnSignIn(snap);
      } else {
        userId = null;
        cloudReady = false;
        applySignedOutUI();
  ...
  // We intentionally delay offline-gain application until AFTER auth sync,
  // because a signed-in session may load cloud state and otherwise "rollback"
  // the offline gain we just applied from local.
  loadLocal();
  normalizeName();

  // Snapshot local timing so auth sync can choose the newer state.
  const localSnapshot = {
    updatedAtMs: Number(state.updatedAtMs || 0),
    lastSeenMs: Number(state.lastSeenMs || 0),
    raw: JSON.parse(JSON.stringify(state))
  ...
  initAuth(localSnapshot).finally(() => {
    // If we're NOT signed in (or cloud sync failed), apply offline from local now.
    // If we are signed in, syncOnSignIn will apply offline exactly once
    // after it resolves the authoritative state.
    if (!cloudReady){
      applyOfflineFromLastSeen("SYS");
      saveLocal();
    }
